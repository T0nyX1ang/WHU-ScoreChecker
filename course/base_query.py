...
# auxiliary function: subtract

def subtract(alist, blist):
...
def search_course_academy(score_table, academy):
	# search academy (precise)
	# may search for a long time
	if academy is None or len(academy) >= 15:
		return []
	result_table = []
	for table_row in score_table:
		if table_row[3] == academy:
			result_table.append(table_row)
	return result_table

def search_course(score_table, course):
	# search course name (precise)
	# may search for a long time
	if course is None or len(course) >= 15:
		return []
	result_table = []
	for table_row in score_table:
		if table_row[0] == course:
			result_table.append(table_row)
	return result_table
