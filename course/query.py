...
def search_start_year(score_table, start_year):
	return [val for val in score_table if (int(val[5]), int(val[6])) >= tuple(start_year)]

def search_stop_year(score_table, stop_year):
	return [val for val in score_table if (int(val[5]), int(val[6])) <= tuple(stop_year)]

def search_score_come_out(score_table, come_out=True):
	if come_out:
		return [val for val in score_table if len(val[7]) != 0]
	else:
		return [val for val in score_table if len(val[7]) == 0]

def search_min_score(score_table, min_score):
	score_come_out = search_score_come_out(score_table, come_out=True)
	return [val for val in score_table if float(val[7]) >= min_score]

def search_max_score(score_table, max_score):
	score_come_out = search_score_come_out(score_table, come_out=True)
	return [val for val in score_table if float(val[7]) <= max_score]

def search_min_credit(score_table, min_credit):
	return [val for val in score_table if float(val[2]) >= min_credit]

def search_max_credit(score_table, max_credit):
	return [val for val in score_table if float(val[2]) <= max_credit]
